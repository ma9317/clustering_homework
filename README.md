...
This analysis successfully demonstrates the application of K-Means and hierarchical clustering to the Scotch whiskey dataset. Both methods identify a robust two-cluster structure that maps intuitively onto well-known stylistic and regional divisions in the Scotch whiskey world. The dendrogram from hierarchical clustering offers additional depth, suggesting sub-cluster structure that could be explored with a finer-grained analysis. The DBSCAN experiment, while not yielding clusters, usefully illustrates the conditions under which density-based methods are and are not appropriate. Together, these results show that unsupervised learning can surface meaningful, domain-relevant groupings from high-dimensional sensory data with no labels required.
The dendrogram in this analysis suggests a natural 2–4 cluster solution, consistent with the K-Means result. The two primary branches likely capture the same broad flavor axes: peated/smoky vs. fruity/sweet. Cutting deeper into the tree could further distinguish, for example, coastal/maritime whiskeys from highland floral styles.  


**Findings**  
Cluster A — Robust, Peated, Full-Bodied Whiskeys that ar characterized by higher rates of peat, smoke, oiliness, and dry or lingering finishes. This profile is consistent with Islay and some Island distilleries, known for their bold, medicinal, and maritime character (e.g., Laphroaig, Ardbeg, Lagavulin).
Cluster B — Light, Sweet, Fruity Whiskeys that are conversely characterized by fresh, fruity, and floral nose notes, lighter body, and sweet palate characteristics. This profile aligns with Speyside and Lowland styles, valued for their approachability and complexity without heavy smoke (e.g., Glenfiddich, Glenlivet, Auchentoshan).
